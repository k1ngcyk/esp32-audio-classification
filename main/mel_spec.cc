...
static float *build_mel_filterbank(int sr, int n_fft, int n_mels, float fmin, float fmax)
{
  ...
  for (int i = 0; i < nbins; ++i)
    freqs[i] = (float)i * (float)sr / (float)n_fft;
  ...
  for (int i = 0; i < npts; ++i)
    mpts[i] = mel_lo + (mel_hi - mel_lo) * (float)i / (float)(npts - 1);
  for (int i = 0; i < npts; ++i)
    hpts[i] = mel_to_hz(mpts[i]);
                    ...
                    int n_mels, float fmin, float fmax)
{
  ...
  // Init FFT tables
  if (dsps_fft2r_init_fc32(NULL, n_fft) != ESP_OK)
  {
    free(ctx);
    return nullptr;
  }
  ...
  if (!ctx->hann_win || !ctx->win_full)
  {
    mel_free(ctx);
    return nullptr;
  }
  if (!build_hann_periodic(win_length, ctx->hann_win))
  {
    mel_free(ctx);
    return nullptr;
  }
  if (!build_centered_window(n_fft, win_length, ctx->hann_win, ctx->win_full))
  {
    mel_free(ctx);
    return nullptr;
  }
